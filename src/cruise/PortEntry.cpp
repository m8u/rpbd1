...
std::vector<PortEntry> PortEntryMapper::get_all() {
    ...
    while (SQL_SUCCEEDED(SQLFetch(db->stmt_handle))) {
        ...
        SQLGetData(db->stmt_handle, 4,
                   SQL_C_TYPE_TIMESTAMP, &sql_destination_ts_actual, 1, nullptr);
        ...
        SQLGetData(db->stmt_handle, 6,
                   SQL_C_TYPE_TIMESTAMP, &sql_departure_ts_actual, 1, nullptr);
        ...
        out_vec.emplace_back(id, port, destination_ts_planned, &destination_ts_actual,
...
void PortEntryMapper::insert(const PortEntry& port_entry) {
    ...
    if (port_entry.destination_ts_actual == nullptr)
        ...
        strftime(destination_ts_actual, 17, "'%Y-%m-%d %H:%M'", port_entry.destination_ts_actual);
    ...
    if (port_entry.departure_ts_actual == nullptr)
        ...
        strftime(departure_ts_actual, 17, "'%Y-%m-%d %H:%M'", port_entry.departure_ts_actual);
    if (port_entry.id == 0) {
        ...
        sprintf(stmt_str, "INSERT INTO ship_port_entries (id, port_id, destination_ts_planned, "
                          ...
                          "VALUES (%d, %d, '%s', %s, '%s', %s, '%s', '%s') ON CONFLICT DO UPDATE SET "
